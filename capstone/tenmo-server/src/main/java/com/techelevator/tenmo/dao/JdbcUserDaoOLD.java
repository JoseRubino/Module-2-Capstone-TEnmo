...
public class JdbcUserDaoOLD implements UserDao {

    private static final BigDecimal STARTING_BALANCE = new BigDecimal("1000.00");
    private JdbcTemplate jdbcTemplate;

    public JdbcUserDaoOLD(JdbcTemplate jdbcTemplate) {
    ...
    @Override
    public User getUserByAccountId(int id) throws AccountNotFoundException {
        User user = new User();

        String sql = "SELECT tenmo_user.user_id FROM tenmo_user" +
                " JOIN account ON tenmo_user.user_id = account.user_id WHERE account.user_id = ?";
        SqlRowSet row = jdbcTemplate.queryForRowSet(sql, id);
        if (row.next()) {
